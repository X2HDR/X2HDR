...
def update_model_with_lora(checkpoint, lora_weights, transformer, cond_width=512, cond_height=512):
        ...
        for name, attn_processor in transformer.attn_processors.items():
            ...
            if name.startswith("transformer_blocks") and layer_index in double_blocks_idx:
                ...
                lora_attn_procs[name] = MultiDoubleStreamBlockLoraProcessor(
                    dim=3072, ranks=ranks, network_alphas=ranks, lora_weights=lora_weights, device=device, dtype=torch.bfloat16, cond_width=cond_width, cond_height=cond_height, n_loras=number
                )
...
def update_model_with_multi_lora(checkpoints, lora_weights, transformer, cond_width=512, cond_height=512):
        ...
        for name, attn_processor in transformer.attn_processors.items():
            ...
            if name.startswith("transformer_blocks") and layer_index in double_blocks_idx:
                ...
                lora_attn_procs[name] = MultiDoubleStreamBlockLoraProcessor(
                    dim=3072, ranks=ranks, network_alphas=ranks, lora_weights=multi_lora_weight, device=device, dtype=torch.bfloat16, cond_width=cond_width, cond_height=cond_height, n_loras=cond_number
                )
...
def set_single_lora(transformer, local_path, lora_weights=[], cond_width=512, cond_height=512):
    checkpoint = load_checkpoint(local_path)
    update_model_with_lora(checkpoint, lora_weights, transformer, cond_width, cond_height)
   
def set_multi_lora(transformer, local_paths, lora_weights=[[]], cond_width=512, cond_height=512):
    checkpoints = [load_checkpoint(local_path) for local_path in local_paths]
    update_model_with_multi_lora(checkpoints, lora_weights, transformer, cond_width, cond_height)
