...
from utils.util_hdr import pu21_decode_rgb, scale_to_L_peak, pu21_encode_rgb
...
class FluxPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def prepare_latents(
        ...
        height_cond = 2 * (self.cond_height // self.vae_scale_factor)
        width_cond = 2 * (self.cond_width // self.vae_scale_factor)
    ...
    def __call__(
        ...
        if sub_number > 0:
            ...
            for subject_image in subject_images:
                ...
                pad_h = cond_height - subject_image.shape[-2]
                pad_w = cond_width - subject_image.shape[-1]
                subject_image = pad(
                    subject_image,
        ...
        if cond_number > 0:
            ...
            for img in spatial_images:
                if input_is_raw:
                    hdr_image, _ = scale_to_L_peak(img)
                    pu21_image = pu21_encode_rgb(hdr_image)
                    pu21_image = torch.from_numpy(np.ascontiguousarray(pu21_image)).permute(2, 0, 1).unsqueeze(0).float()
                    condition_image = pu21_image * 2.0 - 1.0
                else:
                    print(img)
                    condition_image = self.image_processor.preprocess(img, height=self.cond_height, width=self.cond_width)
        ...
        if output_type == "latent":
            ...
            if hdr_mode:
                # image shape [B, C, H, W]
                image_numpy = image.float().permute(0, 2, 3, 1).cpu().numpy()[0]  # [H, W, C]
                pu21_image = np.clip(image_numpy * 0.5 + 0.5, 0, 1)
        ...
        return FluxPipelineOutput(images=image) if not hdr_mode else FluxPipelineOutput(images=image), hdr_image
