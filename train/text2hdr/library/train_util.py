...
from library.hdr_utils import read_hdr_image, pu21_encode_rgb, scale_to_L_peak
...
from library.hdr_utils import pu21_decode_rgb, save_hdr_image
...
IMAGE_EXTENSIONS = [".png", ".jpg", ".jpeg", ".webp", ".bmp", ".PNG", ".JPG", ".JPEG", ".WEBP", ".BMP", ".exr", ".EXR", ".hdr", ".HDR"]
...
# HDR image transform: assumes input is already PU21 encoded in [0,1] range
IMAGE_TRANSFORMS_HDR = transforms.Compose(
    [
...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # Choose image transforms based on HDR mode
        self.hdr_mode = hdr_mode
        self.image_transforms = IMAGE_TRANSFORMS_HDR if hdr_mode else IMAGE_TRANSFORMS
    ...
    def get_image_size(self, image_path):
        ...
        if image_path.endswith(".exr"):
            ...
            return (width, height)
            # FIXME
            # if "1024" in image_path:
            #     return (1024, 1024)
            # elif "512" in image_path:
            #     return (512, 512)
            # else:
            #     print("Please specify the resolution in the folder name.")
    ...
    def load_image_with_face_info(self, subset: BaseSubset, image_path: str, alpha_mask=False):
        img = load_image(image_path, alpha_mask, hdr_mode=self.hdr_mode)
...
class DreamBoothDataset(BaseDataset):
    ...
    def __init__(
        ...
        super().__init__(resolution, network_multiplier, debug_dataset, resize_interpolation, hdr_mode)
    ...
    def __init__(
        ...
        for subset in subsets:
            ...
            for image_key, img_md in metadata.items():
                ...
                caption_long = img_md.get("caption_long", None)
                if caption_long is not None:
                    caption = random.choice([caption, caption_long])
...
def load_image(image_path, alpha=False, hdr_mode=False):
    if hdr_mode:
        hdr_image = read_hdr_image(image_path)
        hdr_image, _ = scale_to_L_peak(hdr_image)
        pu21_image = pu21_encode_rgb(hdr_image)
        return pu21_image
    try:
...
def add_training_arguments(parser: argparse.ArgumentParser, support_dreambooth: bool):
    ...
    # FIXME: uncomment this for SD1.5 training
    # parser.add_argument(
    #     "--hdr_mode",
    #     action="store_true",
    #     help="enable HDR mode for training with .exr format images using PU21 encoding",
    # )
        ...
        with torch.no_grad():
            for prompt_dict in prompts:
                sample_image_inference(
                    accelerator, args, pipeline, save_dir, prompt_dict, epoch, steps, prompt_replacement, controlnet=controlnet, hdr_mode=hdr_mode
                )
    else:
...
def sample_image_inference(
    ...
    if hdr_mode:
        pu21_image = pipeline.decode_latents(latents.to(pipeline.vae.dtype))[0]
        hdr_image = pu21_decode_rgb(pu21_image)
        hdr_image, _ = scale_to_L_peak(hdr_image)
        exr_filename = f"{'' if args.output_name is None else args.output_name + '_'}{num_suffix}_{i:02d}_{ts_str}{seed_suffix}.exr"
        exr_path = os.path.join(save_dir, exr_filename)
