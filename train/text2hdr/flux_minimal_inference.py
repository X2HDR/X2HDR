...
from library.hdr_utils import save_hdr_image, pu21_decode_rgb, scale_to_L_peak, get_luminance_percentile
...
def generate_image(
    ...
    output_path = os.path.join(output_dir, f"{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.png") if file_name is None else os.path.join(output_dir, file_name)
    img.save(output_path)

    # save hdr image
    if args.hdr_mode:
        x_numpy = x.float().cpu().numpy()[0]  # shape: (H, W, C)
        pu21_image = np.clip((x_numpy + 1.0) / 2.0, 0.0, 1.0)
        hdr_image = pu21_decode_rgb(pu21_image)
        hdr_image, _ = scale_to_L_peak(hdr_image)

        # L_peak is 4000.0, so we set a lower bound (target_luminance_peak)
        highest_luminance = get_luminance_percentile(hdr_image)
        scale = args.target_luminance_peak / highest_luminance
        scale = min(scale, 1.0)
        hdr_image = hdr_image * scale

        exr_path = output_path.replace(".png", ".exr")
        save_hdr_image(hdr_image.astype(np.float32), exr_path)

    logger.info(f"Saved image to {output_path}")


def parse_prompt_with_options(prompt_line, default_width, default_height, default_steps, default_guidance, default_cfg_scale, lora_models):
    """
    Parse a prompt line with options like:
    "prompt text --w 512 --h 512 --d 42 --g 3.5 --s 30 --n negative prompt --c 1.5"
    Returns: (prompt, seed, width, height, steps, guidance, negative_prompt, cfg_scale)
    """
        ...
        try:
            ...
            if opt.startswith("w"):
                ...
                if len(multipliers) != len(lora_models):
                    logger.error(f"Invalid number of multipliers, expected {len(lora_models)}, got {len(multipliers)}")
                    continue
    ...
    return prompt, seed, width, height, steps, guidance, negative_prompt, cfg_scale


def process_batch_prompts(batch_prompts_file, model, clip_l, t5xxl, ae, lora_models, args):
    """
    ...
    for i, prompt_line in enumerate(prompts, 1):
        logger.info(f"Processing prompt {i}/{len(prompts)}: {prompt_line[:50]}...")
        
        # Parse prompt with options
        prompt, seed, width, height, steps, guidance, negative_prompt, cfg_scale = parse_prompt_with_options(
            prompt_line, args.width, args.height, args.steps, args.guidance, args.cfg_scale, lora_models
        )
        ...
        try:
            generate_image(
                model, clip_l, t5xxl, ae, prompt, seed, width, height, steps, guidance, negative_prompt, cfg_scale, file_name=f"prompt_{i}.png"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hdr_mode", action="store_true", help="Decode and save HDR image")
    parser.add_argument("--target_luminance_peak", type=float, default=32.0, help="Peak luminance for HDR image")
    parser.add_argument("--batch_prompts", type=str, default=None, help="File containing prompts with options for batch processing")
    ...
    if args.interactive:
        ...
        while True:
            ...
            # parse options
            prompt, seed, width, height, steps, guidance, negative_prompt, cfg_scale = parse_prompt_with_options(
                prompt, width, height, steps, guidance, cfg_scale, lora_models
            )
        ...
        process_batch_prompts(args.batch_prompts, model, clip_l, t5xxl, ae, lora_models, args)
    else:
        # Single prompt mode
        generate_image(
            model,
