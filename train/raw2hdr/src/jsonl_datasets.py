...
import torch
import numpy as np
from src.hdr_utils import read_hdr_image, pu21_encode_rgb, scale_to_L_peak
        ...
        for caption in examples[caption_column]:
            if isinstance(caption, str):
                if random.random() < 0.1:
                    captions.append(" ")  # set text to empty
                else:
...
def make_raw2hdr_train_dataset(args, tokenizer, accelerator=None):
    if args.train_data_dir is not None:
        print("load_raw2hdr_data")
        dataset = load_dataset('json', data_files=args.train_data_dir)
    ...
    def hdr_train_transforms(image):
        image = torch.from_numpy(np.ascontiguousarray(image)).permute(2, 0, 1).float()
        hdr_train_transforms_ = transforms.Compose(
            [
                transforms.Lambda(lambda t: t * 2.0 - 1.0),  # [0,1] -> [-1,1]
            ]
        )
        transformed_image = hdr_train_transforms_(image)
        return transformed_image
    
    def transform_cond_images(images):
        transformed_images = []
        for image in images:
            hdr_tensor = hdr_train_transforms(image)
            transformed_images.append(hdr_tensor)
    ...
    def preprocess_train(examples):
        _examples = {}
        if args.spatial_column is not None:
            spatial_images = [[load_hdr_image(examples[column][i]) for column in spatial_columns] for i in range(len(examples[target_column]))]
            _examples["cond_pixel_values"] = [transform_cond_images(spatial) for spatial in spatial_images]
        target_images = [load_hdr_image(image_path) for image_path in examples[target_column]]
        _examples["pixel_values"] = [hdr_train_transforms(image) for image in target_images]
        _examples["token_ids_clip"], _examples["token_ids_t5"] = tokenize_prompt_clip_t5(examples)
        return _examples
